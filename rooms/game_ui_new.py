        ...
        if os.path.exists(rooms_path):
            for filename in os.listdir(rooms_path):
                if filename.endswith('.png'):
                    ...
                    # Corrections spécifiques de noms mal normalisés (alias)
                    # Exemple: VIOLETHer_Ladys_Chamber_Icon.png → "Her Ladys Chamber"
                    # alors que la pièce s'appelle "Her Lady's Chamber" dans le catalogue
                    name_fixes = {
                        "Her Ladys Chamber": "Her Lady's Chamber",
                    }
                    # Appliquer les corrections connues
                    fixed_name = name_fixes.get(name, name)
                    # Correction générique pour "Ladys" → "Lady's" si d'autres fichiers similaires existent
